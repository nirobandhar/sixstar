...
class Products extends CI_Controller {
    ...
    public function insert_product()  {
			...
			"sizeId"                       =>$this->input->post('size', TRUE),
            ...
            "Product_WholesaleRate"        =>$this->input->post('Wholesales', TRUE),
	...
	public function fanceybox_size()  {
        $this->load->view('products/fanceybox_size');
    }
    public function insert_fanceybox_size()  {
        ...
        if($query->num_rows() > 0){
            ...
            $this->load->view('ajax/fanceybox_product_size');
        }
    }
	...
	public function branch_current_stock()  {
        $data['title'] = "Current Stock";
        $data['content'] = $this->load->view('stock/branchcurrent_stock', $data, TRUE);
        $this->load->view('index', $data);
    }
	...
	function transfer(){
        ...
        $data['content'] = $this->load->view('products/transfer', $data, TRUE);
        $this->load->view('index', $data);
		}
	function ptransfer(){
        $this->load->view('products/ptransfer');
		}
   function transfersuccess(){
	 ...
	 function pending(){
        ...
        $data['content'] = $this->load->view('products/pending_list', $data, TRUE);
        $this->load->view('index', $data);
		}
	 function sentlist(){
		 $data['title'] = "Product Transfer Sent";
        $data['content'] = $this->load->view('products/sentlist', $data, TRUE);
        $this->load->view('index', $data);
		}
	function transfer_req(){
		 $data['title'] = "Product Transfer Request";
        $data['content'] = $this->load->view('products/transfer_requiest', $data, TRUE);
        $this->load->view('index', $data);
		}
	...
	function transferreject(){
	  ...
	  function transfer_record(){
        ...
        $data['content'] = $this->load->view('products/transrecord', $data, TRUE);
	...
	function branchwisestock(){
        ...
        <h4><a style="cursor:pointer" onclick="window.open('<?php echo base_url();?>reports/print_current_stock?brid=<?php echo $tobranchname;?>', 'newwindow', 'width=850, height=800,scrollbars=yes'); return false;"><img src="<?php echo base_url(); ?>images/printer.png" alt=""> Print</a></h4>
        <tr>
